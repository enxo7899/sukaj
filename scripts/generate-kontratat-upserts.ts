...
function main() {
  ...
  if (!inputPathArg) {
    ...
    console.log('Usage: tsx scripts/generate-kontratat-upserts.ts "/absolute/path/to/Kontrata Qeraje.xlsx"');
    process.exit(1);
  }
  ...
  // Deduplicate by (nr_repert, nr_koleks) keeping the last occurrence
  const map = new Map<string, Record<string, any>>();
  for (const r of data) {
    const row = makeRow(r);
    if (!row.qera_marres && !row.qera_dhenes && !row.nr_apartament) continue;
    const key = `${row.nr_repert ?? ''}__${row.nr_koleks ?? ''}`;
    map.set(key, row);
  }

  if (map.size === 0) {
    console.error('⚠️ No valid rows to upsert.');
    process.exit(1);
  }
  ...
  for (const row of map.values()) {
    ...
    const stmt = `INSERT INTO public.kontratat ("${columns.join('", "')}")\nVALUES (${vals.join(', ')})\nON CONFLICT (nr_repert, nr_koleks) DO UPDATE SET ${updateSet};`;
